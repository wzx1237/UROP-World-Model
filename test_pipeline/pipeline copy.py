...
def main() -> int:
    ...
    if not args.skip_moregen:
        ...
        # Run all but the last MoreGen command as background services,
        # then run the final command and clean up the background ones.
        background_procs = []
        try:
            if len(moregen_commands) >= 2:
                for command in moregen_commands[:-1]:
                    rendered = render_template(
                        command,
                ...
                # run the last command blocking
                last_cmd = moregen_commands[-1]
                rendered_last = render_template(
                    last_cmd,
                    moregen_dir=str(moregen_dir),
                    meshes_dir=str(meshes_dir),
                    results_dir=str(results_dir),
                ...
                run_in_conda(rendered_last, cwd=moregen_dir, conda_env=args.moregen_env, dry_run=args.dry_run)
            else:
            ...
            if not args.dry_run:
                for p in background_procs:
                    if p is None:
                        continue
                    if p.poll() is None:
                        print(f"[CLEANUP] terminating background pid={p.pid}")
                        try:
                            p.terminate()
                            p.wait(timeout=5)
                        except Exception:
