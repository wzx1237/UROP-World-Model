    ...
    if [ "$PLATFORM" = "cuda" ] ; then
        if [ "$CUDA_VERSION" = "11.8" ] ; then
            case $PYTORCH_VERSION in
                ...
                2.4.0+cu118) pip install xformers==0.0.27.post2 --index-url https://download.pytorch.org/whl/cu118 ;;
            ...
            case $PYTORCH_VERSION in
                ...
                2.4.0+cu121) pip install xformers==0.0.27.post2 --index-url https://download.pytorch.org/whl/cu121 ;;
...
if [ "$KAOLIN" = true ] ; then
    ...
    if [ "$PLATFORM" = "cuda" ] ; then
        case $PYTORCH_VERSION in
            ...
            2.4.0+cu118) pip install kaolin -f https://nvidia-kaolin.s3.us-east-2.amazonaws.com/torch-2.4.0_cu121.html;;
