...
PORT = 23457      # Arbitrary non-privileged port
IMG_WIDTH = 420
IMG_HEIGHT = 270
...
def get_frame_list(output_path, start_frames=0, frames_to_keep=48):
    ...
    available = max(0, total_frames - start_frames)
    if available == 0:
        return []
    frames_to_keep = min(frames_to_keep, available) if frames_to_keep > 0 else available
    # Avoid step==0 when available < frames_to_keep
    step = max(1, available // max(1, frames_to_keep))
...
def handle_client(conn, addr):
    ...
    try:
        ...
        if purpose == 'feedback':
            # Backward/forward compatible request parsing:
            # - legacy: {'prompt': str, 'video_path': str}
            # - newer:  {'feedback_prompt': str, 'original_prompt': str, 'frames_path': str}
            feedback_prompt = req.get('feedback_prompt') or req.get('prompt')
            original_prompt = req.get('original_prompt')
            video_path = req.get('video_path')
            frames_path = req.get('frames_path')

            # Prefer frames when provided (headless PyBullet).
            if frames_path:
                res, phys_result = query_video(feedback_prompt, use_frames=True, frames_path=frames_path)
                if original_prompt:
                    res, intent_result = query_video(original_prompt, use_frames=True, frames_path=frames_path)
                else:
                    intent_result = None
            else:
            ...
            result = {
                "visual_feedback_physics": phys_result,
